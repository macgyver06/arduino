...
import pyautogui # c'est la librairie qui possède les fonctions controlant le clavier et la souris

ArduinoSerial = serial.Serial('/dev/cu.usbserial-1410',9600) #Crée un port série 
...
# si on appuie sur le bouton cliquer dans l'appli Bluetooth Electronics
    if 'CLICK' in incoming:
        pyautogui.click()  # simule un clic sur le trackpad de l'ordi
        


# si on appuie de maniere ponctuelle sur le PAD
    if (incoming[0:4] == "b'MX") :
        gauche,droite = incoming.split('Y')
        Y,b = droite.split("\\r")
        a,X = gauche.split('X')
        pyautogui.moveTo(int(X), int(Y)) # simule un déplacement de la souris vers le point (X,Y)
